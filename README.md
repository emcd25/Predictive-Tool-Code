...
Finally, the script "V2_Predictive_Tool.m" will provide estimates for all of the injury metrics for the specified impact case. Given the impact location, PLV and PAV, this script calls each metric-specific predictive tool - "MPS_Predictive", "MPS95_Predictive", "MPSR95_Predictive", "MPSRxSR95_Predictive" and "VMS95_Predictive". Each of these then apply the corresponding location-specific script ( e.g., "MPS_FRONT_HIGH.m", "MPS95_FRONT_HIGH.m", etc.). The output is a table containing regional estimates of all injury metrics for the specified impact case.
